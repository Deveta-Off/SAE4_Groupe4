...
io.on("connection", (socket) => {
  ...
  //Mise à jour du status d'écriture
  let currentlyWritingNames = [];
  socket.on("isWritingUpdate", (isWriting) => {
    let rooms = Array.from(socket.rooms);
    if (isWriting) {
      currentlyWritingNames.push(socket.request.session.username);
    } else {
      currentlyWritingNames = currentlyWritingNames.filter(
        (el) => el != socket.request.session.username
      );
    }
    if (rooms.length > 1) {
      const roomName = rooms[1];
      socket.broadcast.to(roomName).emit("usersWritingUpdate", currentlyWritingNames);
    } else {
      socket.broadcast.to("general").emit("usersWritingUpdate", currentlyWritingNames);
    }
  });

  socket.on("joinClass", (e) => {
    socket.leave("general");
    socket.join(socket.request.session.category);
  });

  socket.on("leaveClass", (e) => {
    socket.join("general");
    socket.leave(socket.request.session.category);
  });
});
